...
(function () {
  function adjustButtonPositionNearFooter() {
    ...
    // If footer is visible or close to viewport (within 600px), move buttons up to 5rem
    // Otherwise keep them at 1rem
    const shouldMoveButtonsUp = distanceToFooter < 600;
    
    if (shouldMoveButtonsUp) {
      // Move buttons up to 5rem from bottom
      autoScrollBtn.style.bottom = '13rem';
      backToTopBtn.style.bottom = '13rem';
    } else {
      // Keep buttons at default 1rem from bottom (as per media query)
      autoScrollBtn.style.bottom = '';
